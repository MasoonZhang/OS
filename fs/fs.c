...
char* sys_getcwd(char* buf, uint32_t size) {
    ...
    int32_t childe_inode_nr = cur_thread->cwd_inode_nr;
    ASSERT(childe_inode_nr >= 0 && childe_inode_nr < 4096);
    if (childe_inode_nr == 0) {
        buf[0] = '/';
        buf[1] = 0;
	sys_free(io_buf);
        return buf;
    ...
    while ((childe_inode_nr)) {
        parent_inode_nr = get_parent_dir_inode_nr(childe_inode_nr, io_buf);
        if (get_child_dir_name(parent_inode_nr, childe_inode_nr, full_path_reverse, io_buf) == -1) {
            sys_free(io_buf);
            return NULL;
        }
        childe_inode_nr = parent_inode_nr;
    }
